...
### 2. Stripe API キーを設定する

管理画面 → **キチキチ 決済予約** → **設定タブ** を開き、以下を入力して保存します。

| 項目 | 説明 |
|---|---|
| Stripe 公開キー | `pk_live_...` または `pk_test_...` |
| Stripe シークレットキー | `sk_live_...` または `sk_test_...` |
| Stripe Webhook シークレット | `whsec_...`（Stripe ダッシュボードで取得） |
| 送信元メールアドレス | 確認メールの From に使用 |
| 送信元名 | 確認メールの差出人名（例: キチキチ） |
...
> Webhook シークレット（`whsec_...`）を取得して管理画面の設定タブに入力してください。
...
### wp_options に保存される設定値

| オプション名 | 内容 |
|---|---|
| `kkpay_stripe_pk` | Stripe 公開キー |
| `kkpay_stripe_sk` | Stripe シークレットキー |
| `kkpay_stripe_webhook_secret` | Webhook 署名シークレット |
...
Stripe API キーとメール送信設定を入力・保存します。
...
1. Stripe ダッシュボードで Webhook が正常に届いているか確認
2. 管理画面の設定タブで Webhook シークレット（`whsec_...`）が正しく入力されているか確認
3. WordPress の REST API が有効か確認（`https://yoursite.com/wp-json/` にアクセスして JSON が返れば OK）

### メールが届かない

- WP Mail SMTP プラグインの **Email Test** 機能でテスト送信を実施してください
- 管理画面の設定タブで「送信元メールアドレス」が Xserver に登録済みのアドレスと一致しているか確認してください
...
- Stripe 公開キー（`pk_`）が管理画面の設定タブに入力されているか確認してください
