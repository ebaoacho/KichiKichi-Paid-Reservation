...
class KKPAY_Mailer {
    ...
    public static function send_cancellation_confirmation( $reservation, $refund_status, $refund_amount ) {
        ...
        if ( $refund_status === 'full' ) {
            ...
            $bodies = array(
                'en' => "Dear {$reservation->name},\n\n"
                    . "Your reservation has been cancelled and a full refund of ¥{$refund_amount} has been processed.\n\n"
                    . "Reservation Date : {$reservation->reservation_date}\n"
                    . "Time Slot        : {$label}\n\n"
                ...
                'ja' => "{$reservation->name} 様\n\n"
                    . "ご予約をキャンセルし、¥{$refund_amount} の全額返金処理を行いました。\n\n"
                    . "予約日  : {$reservation->reservation_date}\n"
                    . "時間枠  : {$label}\n\n"
                    . "返金は5〜10営業日以内に反映されます。\n\n"
                    . "キチキチ",
                ...
                'ko' => "{$reservation->name} 님\n\n"
                    . "예약이 취소되었으며 ¥{$refund_amount} 전액 환불 처리가 완료되었습니다.\n\n"
                    . "예약 날짜 : {$reservation->reservation_date}\n"
                    . "시간대    : {$label}\n\n"
    ...
    private static function send( $to, $subject, $message ) {
        $from_name  = get_option( 'kkpay_from_name', 'KichiKichi' );
        $from_email = get_option( 'kkpay_from_email', get_option( 'admin_email' ) );
