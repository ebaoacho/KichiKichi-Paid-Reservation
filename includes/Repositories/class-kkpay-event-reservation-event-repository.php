...
/**
 * kkpay_event_reservation_events テーブル（Event Reservation 専用の監査ログ）へのアクセスを担当する。
 * 通常予約の KKPAY_Reservation_Event_Repository と同じ構造だが、完全に別テーブル・別クラスとして分離する。
 */
class KKPAY_Event_Reservation_Event_Repository {

    private static function table() {
        global $wpdb;
        return $wpdb->prefix . 'kkpay_event_reservation_events';
    }

    public static function insert( $reservation_id, $hold_token, $payment_intent_id, $event_type, $actor_type, $payload, $actor_id = null ) {
        global $wpdb;
        ...
        $row = array(
            'reservation_id'    => $reservation_id !== null ? (int) $reservation_id : null,
        ...
        return $inserted
            ? (int) $wpdb->insert_id
            : new WP_Error( 'db_insert_failed', 'kkpay_event_reservation_events insert failed: ' . $wpdb->last_error );
    }
