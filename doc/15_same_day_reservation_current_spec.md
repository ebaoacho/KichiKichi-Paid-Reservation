...
| 現在時刻 | 許可される枠 |
| --- | --- |
| 09:30以上 11:59以下 | `slot_1`, `slot_2` |
| 13:30以上 15:59以下 | `slot_3`, `slot_4`, `slot_5`, `slot_6` |
| 上記以外 | 予約不可 |

この判定は統合後、`KKPAY_Same_Day_Reservation_Service` のサーバー時刻（JST）で再検証される。  
...
ただしサーバー側の検証は既存プラグインの `pattern="[A-Za-z\s]+"` より少し広くし、共通規約に合わせてスペース・ドット・アポストロフィ・ハイフンを許可する。PR 5 で追加するフロント側 pattern もこのサーバー側ルールに合わせる。
