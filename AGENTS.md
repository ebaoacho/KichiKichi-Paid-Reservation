...
| `[kkpay_event_reservation]` | `templates/event-reservation-page.php` | Limited Event Reservation form (English only) |
| `[kkpay_event_cancel]` | `templates/event-cancel.php` | Event Reservation self-cancellation via reservation code + email; no refund |
...
| `kkpay_event_get_available_slots` | `KKPAY_Event_Reservation_Controller::ajax_get_available_slots` |
| `kkpay_event_create_hold` | `KKPAY_Event_Reservation_Controller::ajax_create_hold` |
| `kkpay_event_confirm_reservation` | `KKPAY_Event_Reservation_Controller::ajax_confirm_reservation` |
| `kkpay_event_check_reservation` | `KKPAY_Event_Reservation_Controller::ajax_check_reservation` |
| `kkpay_event_cancel_reservation` | `KKPAY_Event_Reservation_Controller::ajax_cancel_reservation` |
...
| `kkpay_event_export_csv` | `KKPAY_Event_Reservation_Controller::ajax_export_csv` |
| `kkpay_event_save_status` | `KKPAY_Event_Reservation_Controller::ajax_save_status` |
| `kkpay_event_admin_cancel` | `KKPAY_Event_Reservation_Controller::ajax_admin_cancel` |
...
### Event Reservation Exception

Event Reservation では、通常予約と同じ「返金は一切行わない」ルールを適用するが、キャンセル手段は通常予約と異なる。

- 顧客自身が `[kkpay_event_cancel]`（`reservation_code` + `email` で照合）から、開催日時を過ぎるまではいつでもキャンセルできる。開催日時を過ぎた予約は、混乱防止のためオンラインでのキャンセルを許可しない（管理画面からスタッフが個別対応する）。
- 管理画面（`イベント予約`タブ）からもスタッフが無条件にキャンセルできる（`KKPAY_Event_Reservation_Service::admin_cancel()`）。
- どちらの経路も `reservation_status` を `CANCELED` にするのみで、`payment_status` は変更しない。Stripe `/v1/refunds` を呼ぶコードパスはシステム内に存在しない（`admin_refund()` は廃止済み）。
- `charge.refunded` Webhook（`KKPAY_Event_Reservation_Service::sync_external_refund()`）は、チャージバックや Stripe ダッシュボードでの手動対応など、本システムの外側で返金が発生した場合に限り DB の記録を同期するためのものであり、本システムが返金を実行するものではない。
